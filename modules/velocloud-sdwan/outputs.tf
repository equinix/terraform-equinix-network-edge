...
output "secondary" {
  ...
  value = var.secondary.enabled ? {
    id             = equinix_network_device.non_cluster.secondary_device[0].uuid
    status         = equinix_network_device.non_cluster.secondary_device[0].status
    license_status = equinix_network_device.non_cluster.secondary_device[0].license_status
    account_number = equinix_network_device.non_cluster.secondary_device[0].account_number
    region         = equinix_network_device.non_cluster.secondary_device[0].region
    ibx            = equinix_network_device.non_cluster.secondary_device[0].ibx
    ssh_ip_address = equinix_network_device.non_cluster.secondary_device[0].ssh_ip_address
    ssh_ip_fqdn    = equinix_network_device.non_cluster.secondary_device[0].ssh_ip_fqdn
    interfaces     = equinix_network_device.non_cluster.secondary_device[0].interface
  } : null
}
